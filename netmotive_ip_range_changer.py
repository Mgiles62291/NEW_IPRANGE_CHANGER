...
def run_netsh(cmd: list[str]) -> str | None:
    """Execute a netsh command, surface stderr if it fails."""
    try:
...
def get_adapters() -> list[str]:
    """Return interface names (spaces preserved)."""
    out = subprocess.check_output("netsh interface show interface", shell=True, text=True)
    adapters = [" ".join(l.split()[3:]) for l in out.splitlines()
                if any(t in l for t in ("Dedicated", "Ethernet", "Wi-Fi"))]
...
def static_cmd(adapter: str, p: dict):
    return ["netsh", "interface", "ip", "set", "address", q(adapter), "static",
            p["ip"], p["mask"], p["gateway"], "1"]

def dns_cmd(adapter: str, dns: str, dhcp: bool):
    return (["netsh", "interface", "ip", "set", "dns", q(adapter), "dhcp"] if dhcp else
...
def apply_profile(adapter: str, profile):
    ...
    run_netsh(static_cmd(adapter, profile)); run_netsh(dns_cmd(adapter, profile["dns"], False))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        btn = QHBoxLayout()
        for b in (self.applyBtn, self.addBtn, self.updateBtn, self.deleteBtn, self.dhcpBtn): btn.addWidget(b)
        root.addLayout(btn)
    ...
    # ------------ list helpers ------------
    def _refresh(self):
        self.profileList.clear(); self.profileList.addItems(self.profiles.keys())

    def _load(self, item: QListWidgetItem):
        name=item.text(); self.profileName.setText(name)
        prof=self.profiles[name]
        if prof=="dhcp":
            for e in (self.ip,self.mask,self.gw,self.dns): e.clear(); return
        self.ip.setText(prof["ip"]); self.mask.setText(prof["mask"])
        self.gw.setText(prof["gateway"]); self.dns.setText(prof["dns"])

    def _collect(self):
        return {"ip":self.ip.text().strip(),"mask":self.mask.text().strip(),"gateway":self.gw.text().strip(),"dns":self.dns.text().strip()}

    # CRUD
    def _add(self):
        n=self.profileName.text().strip();
        if not n: QMessageBox.warning(self,APP_NAME,"Name required"); return
        if n in self.profiles: QMessageBox.warning(self,APP_NAME,"Profile exists – use Update"); return
    ...
    def _apply(self):
        it=self.profileList.currentItem();
        if not it: QMessageBox.information(self,APP_NAME,"Select a profile"); return
